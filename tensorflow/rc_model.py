...
class RCModel(object):
    ...
    def __init__(self, vocab, brc_data, args):

        # logging
        self.logger = logging.getLogger("brc")
        self.brc_data = brc_data
        # basic config
        self.algo = args.algo
        ...
        # save info
        # 模型保存后生成四个文件：存储网络结构.meta、存储训练好的参数.data和.index、记录最新的模型checkpoint
    ...
    def _setup_placeholders(self):
        ...
        Placeholders
        """
        # self.p = tf.placeholder(tf.float32, [None, None, 768])
        # self.q = tf.placeholder(tf.float32, [None, None, 768])
        self.p = tf.placeholder(tf.float32, [None, None, 1024])
        self.q = tf.placeholder(tf.float32, [None, None, 1024])
    ...
    def _decode(self):
        ...
        使用指针网络获取每个位置的概率问题
        是预测答案的开始或结束。
        注意，我们注意到fuse_p_encodes用于同一文档中的段落。
        由于同一文档中的查询编码是相同的，所以我们选择第一个。
    ...
    def _train_epoch(self, train_batches, dropout_keep_prob):
        ...
        # 这里是一个迭代器
        line_number = 100 # 从第1行开始读
        last_loss = 7.562585
        for bitx in range(1,8000):
            self.logger.info('bitx: '+ str(bitx))
            batch, line_number= self.brc_data.load_batch_size_data_set(line_number, '../data/trainset/search.train.json',
                                                                   train=True)
            # 传入7个数据
            ...
            if loss < 3:
                self.save('../data/models/3/', 'bidaf')
                self.logger.info('save---model--- ')
            elif loss < 4:
                self.save('../data/models/4/', 'bidaf')
                self.logger.info('save---model--- ')
            elif loss < 5:
                self.save('../data/models/5/', 'bidaf')
                self.logger.info('save---model--- ')
            elif loss < 6:
                self.save('../data/models/6/', 'bidaf')
                self.logger.info('save---model--- ')
            elif loss < 7:
                self.save('../data/models/7/', 'bidaf')
                self.logger.info('save---model--- ')
            elif loss < 8:
                self.save('../data/models/8/', 'bidaf')
                self.logger.info('save---model--- ')
            if loss < last_loss:
                last_loss = loss
                self.save('../data/models/', 'bidaf')
                self.logger.info('save---model--- ')
        ...
        for epoch in range(1, epochs + 1):
            ...
            if evaluate:
                ...
                if data.dev_set is not None:
                    ...
                    # eval_batches = self.brc_data.load_batch_size_data_set(epoch, '../data/devset/search.dev.json')
                    eval_batches = ''
    ...
    def evaluate(self, eval_batches, result_dir=None, result_prefix=None, save_full_info=False):
        ...
        for bitx in range(1,72):
            batch, line_number = self.brc_data.load_batch_size_data_set(line_number, '../data/devset/search.dev.json',
                                                                   train=True)
            feed_dict = {self.p: batch['passage_token_ids'],
                         self.q: batch['question_token_ids'],
                         self.p_length: batch['passage_length'],
